...
const Navbar: React.FC<NavbarProps> = ({ isDarkMode, toggleTheme }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-700 ${isScrolled ? (isDarkMode ? 'bg-black/40 border-white/10' : 'bg-white/40 border-white/40') : 'bg-transparent py-10'} backdrop-blur-2xl py-4 border-b`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        <a href="#" className="flex items-center gap-3 group transition-transform duration-500 hover:scale-105">
          <img 
            src="https://i.imgur.com/GWHQ6Y2.png" 
            alt="Avaeventos Logo" 
            className={`h-12 md:h-16 w-auto object-contain transition-all duration-700 ${isDarkMode ? 'brightness-125 contrast-125' : 'brightness-100'}`}
          />
          <div className="hidden sm:block">
            <span className={`text-xl font-bold tracking-[0.3em] transition-colors duration-700 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="lg:hidden flex items-center space-x-4">
          <button onClick={toggleTheme} className={`p-2 rounded-full ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
             {isDarkMode ? <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg> : <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>}
          </button>
          <button className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
