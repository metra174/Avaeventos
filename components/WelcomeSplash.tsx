...
const WelcomeSplash: React.FC<WelcomeSplashProps> = ({ onEnter, isDarkMode }) => {
  ...
  useEffect(() => {
    // Mantemos em 100ms para o design aparecer imediatamente
    mountTimerRef.current = setTimeout(() => {
      setMounted(true);
    }, 100);

    // Inicia a saída aos 20 segundos
    autoExitTimerRef.current = setTimeout(() => {
      setIsExiting(true);
    }, 20000);

    // Finaliza aos 20.9 segundos (20s + duração da transição de 900ms)
        ...
        >
          ...
          <div className="relative h-[2px] w-32 bg-white/10 mx-auto my-6 overflow-hidden rounded-full">
            <div 
              ...
              // A barra de progresso agora vai demorar 20 segundos para completar
